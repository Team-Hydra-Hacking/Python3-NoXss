...
def change_by_param(url, param, tovalue):
    ...
    parsed_dict = dict([(k, v[0]) for k, v in list(urllib.parse.parse_qs(url_parsed.query).items())])
    parsed_dict[param] = tovalue
    new_url = url.split("?")[0] + '?' + urllib.parse.urlencode(parsed_dict)
    return new_url
...
def cookiedict2str(cookie_dict):
    cookiestr=''
    for key,value in list(cookie_dict.items()):
        cookiestr+=key+'='+value+';'+' '
    return cookiestr
...
def gen_id():
    return ''.join([(hex(ord(xx))[2:]) for xx in os.urandom(8)])
...
def save(result,id):
    ...
    if result:
        for vul,location,poc in result:
            ...
            if vul in list(result_dict.keys()):
