...
class Detector():
    ...
    def detect_param(request):
        ...
        if method == 'GET':
            ...
            param_dict = dict([(k, v[0]) for k, v in list(urllib.parse.parse_qs(url_parsed.query).items())])
        elif method == 'POST':
            if body == '':
                return param_dict
            # {a:1}
...
class Processor():
    ...
    def process_reflect(self):
        for param, value in list(self.param_dict.items()):
            # improve accuracy
            if len(value) > 1:
    ...
    def get_process_chains():
        return set(list(
            [m for m in dir(Processor) if not m.startswith("__") and not m.endswith("__") and callable(getattr(Processor, m))])) - {'run', 'get_process_chains', }
